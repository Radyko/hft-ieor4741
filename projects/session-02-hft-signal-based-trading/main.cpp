...
public:
    explicit MarketDataFeed(std::vector<MarketData>& ref) : data(ref) {}

    void generateData(int num_ticks) const
    {
...
public:
    ...
    void reportStats() {
        ...
        std::cout << "Total Market Ticks Processed: " << market_data.size() << std::endl;
        std::cout << "Total Orders Placed: " << orders.size() << std::endl;
        std::cout << "Average Tick-to-Trade Latency (ns): " << (latencies.empty() ? 0 : sum / latencies.size()) << std::endl;
        std::cout << "Maximum Tick-to-Trade Latency (ns): " << max_latency << std::endl;
        std::cout << "Signal 1: " << signal1count << std::endl;
        std::cout << "Signal 2: " << signal2count << std::endl;
        std::cout << "Signal 3: " << signal3count << std::endl;
    }
